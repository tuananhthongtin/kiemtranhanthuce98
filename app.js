document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    ...
    if (storedHistory) {
      testHistory = JSON.parse(storedHistory);
    }
  } catch (e) {
  ...
  function nopBai(bypassConfirm = false) {
    ...
    if (!result) {
      ...
      alert("Lỗi giao diện. Vui lòng liên hệ admin.");
      return;
    }
    ...
    document.getElementById("settingsBtn").style.display = isAdmin
      ? "inline-block"
      : "none";
    document.getElementById("historyBtn").style.display = isPracticeMode
      ? "none"
      : "inline-block";
    hienThiCauHoi();
  }
  ...
  function saveQuestion() {
    ...
    if (checkLocalStorageCapacity(questions)) {
      ...
      alert("Dung lượng localStorage đã đầy, không thể lưu thêm câu hỏi!");
    }
  ...
  function checkLocalStorageCapacity(data) {
    try {
      ...
      alert("Lỗi lưu trữ dữ liệu. Vui lòng liên hệ admin.");
      return false;
    }
  }
